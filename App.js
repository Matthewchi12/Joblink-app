...
const demoJobs = [
  {
    ...
    description: "Demo job - apply to real jobs from database.",
...
export default function App() {
  ...
  async function loadEmployerApplications() {
    try {
      setAppsLoading(true);
      const res = await fetch(`${BACKEND_URL}/api/employer/applications`, {
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    const l = locationSearch.toLowerCase().trim();
  ...
  function handleJobApply() {
    ...
    if (selectedJob.id?.toString().startsWith("demo-")) {
      Alert.alert("Demo Job", "Please apply to real jobs from database.");
      return;
    }
    if (!user ||!accessToken) {
    ...
    if (user.role === "employer") {
      Alert.alert("HR Account", "Employers cannot apply.");
      return;
    }
    // EXTERNAL JOBS
    if (selectedJob.application_url) {
      Alert.alert("External Application", `This job requires you to apply on company website:\n${selectedJob.application_url}`, [
  ...
  async function submitApplication() {
    if (!appName.trim() ||!appEmail.trim() ||!appPhone.trim()) {
      Alert.alert("Missing", "Name, Email and Phone are required.");
      return;
    }
    if (!appCv.trim()) {
      Alert.alert("Missing", "Please paste your CV link (Google Drive link).");
      return;
    }
    try {
      setApplying(true);
      const res = await fetch(`${BACKEND_URL}/api/jobs/${selectedJob.id}/apply`, {
        ...
        body: JSON.stringify({
          ...
          phone: appPhone.trim(),
          cv_url: appCv.trim(),
          cover_letter: appCover.trim(),
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed");
      Alert.alert("Success 🎉", "Application sent! Employer will see your CV, phone and email.");
      setShowApplyForm(false);
      setScreen("home");
    } catch (e) {
  ...
  async function submitJob() {
    if (!user ||!accessToken || user.role!== "employer") {
      Alert.alert("Employer login required");
      setAuthRole("employer"); setAuthScreen("login"); setScreen("auth");
      return;
    }
    if (!jobTitle.trim() ||!companyName.trim() ||!jobLocation.trim() ||!jobDescription.trim()) {
      Alert.alert("Missing", "Fill all required fields.");
      return;
    }
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed");
      Alert.alert("Submitted", "Job waiting for approval.");
      setJobTitle(""); setCompanyName(""); setJobLocation(""); setJobSalary(""); setJobDescription(""); setApplicationEmail(""); setApplicationUrl("");
    ...
    return (
      <SafeAreaView style={styles.container}>
        <ScrollView contentContainerStyle={styles.authContainer}>
          ...
          <TouchableOpacity onPress={() => setAuthScreen(authScreen === "login"? "signup" : "login")}>
            <Text style={styles.authSwitch}>{authScreen === "login"? "Don't have account? Sign up" : "Have account? Login"}</Text>
          </TouchableOpacity>
        </ScrollView>
      </SafeAreaView>
    );
      ...
      return (
        <SafeAreaView style={styles.container}>
          <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
            ...
            <Text style={styles.formSubtitle}>Employer will receive your CV and contact.</Text>
            ...
            <FormLabel text="CV Link (Google Drive / Dropbox) *" /><TextInput style={styles.input} value={appCv} onChangeText={setAppCv} placeholder="https://drive.google.com/..." autoCapitalize="none" />
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.detailsContent}>
          ...
          {selectedJob.application_url? <InfoRow icon="🔗" text={`External: ${selectedJob.application_url}`} /> : null}
          {selectedJob.application_email? <InfoRow icon="📧" text={`Email: ${selectedJob.application_email}`} /> : null}
          <Text style={styles.sectionHeading}>About</Text><Text style={styles.description}>{selectedJob.description}</Text>
          <TouchableOpacity style={styles.applyButton} onPress={handleJobApply}>
            <Text style={styles.applyText}>{selectedJob.application_url? "Apply on Company Site →" : "Apply for Free →"}</Text>
          </TouchableOpacity>
        </ScrollView>
    ...
    return (
      <SafeAreaView style={styles.container}>
        <ScrollView contentContainerStyle={styles.content}>
          ...
          {appsLoading? <ActivityIndicator /> : employerApps.length === 0? <EmptyState icon="📭" title="No applications yet" text="When seekers apply with CV, they will appear here." /> :
            employerApps.map((app) => (
              <View key={app.id} style={styles.jobCard}>
                ...
                <Text style={styles.jobLocation}>🔗 CV: {app.cv_url || "No CV"}</Text>
                {app.cv_url? <TouchableOpacity onPress={() => Linking.openURL(app.cv_url)}><Text style={{ color: "#2563EB", fontWeight: "800", marginTop: 6 }}>Open CV →</Text></TouchableOpacity> : null}
  ...
  if (screen === "employerDashboard") {
    return (
      <SafeAreaView style={styles.container}>
        <ScrollView contentContainerStyle={styles.content}>
          <Text style={styles.logo}>JobLink</Text>
          <Text style={styles.pageTitle}>HR Dashboard</Text>
          <Text style={styles.dashboardWelcome}>Welcome, {user?.full_name || "Employer"}</Text>
          <View style={styles.hrCard}><Text style={styles.hrCardTitle}>👔 {user?.email}</Text><Text style={styles.hrCardText}>Post external link jobs OR receive applications inside app with CV.</Text></View>
          ...
          <TouchableOpacity style={[styles.dashboardButton, { backgroundColor: "#111827", marginTop: 12 }]} onPress={() => { loadEmployerApplications(); setScreen("employerApps"); }}><Text style={styles.dashboardButtonText}>👥 View Applications ({employerApps.length})</Text></TouchableOpacity>
    ...
    return (
      <SafeAreaView style={styles.container}>
        <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
          ...
          <Text style={styles.formSubtitle}>Leave URL empty to receive applications inside JobLink with CV. Fill URL to redirect to company site.</Text>
          ...
          <TouchableOpacity style={[styles.applyButton, postingLoading && styles.disabledButton]} onPress={submitJob} disabled={postingLoading}>
            {postingLoading? <ActivityIndicator color="#fff" /> : <Text style={styles.applyText}>Submit Job</Text>}
          </TouchableOpacity>
        </ScrollView>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.topBar}><View><Text style={styles.logo}>JobLink</Text><Text style={styles.tagline}>No login needed to browse</Text></View>
        ...
        <View style={styles.hero}><Text style={styles.heroTitle}>Find a job you'll love.</Text><Text style={styles.heroSubtitle}>Discover opportunities. No signup needed to browse.</Text></View>
...
const styles = StyleSheet.create({
  ...
  authSubtitle: { textAlign: "center", color: "#6B7280", marginTop: 8, marginBottom: 20 },
  ...
  logoutButton: { borderColor: "#FECACA", marginTop: 25 },
